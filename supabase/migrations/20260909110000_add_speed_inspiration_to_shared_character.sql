...
-- 20260908093000/094500/100000/103000) : ajoute `inspiration` et
-- `race_speed` au jsonb `character` retourné par public.get_shared_character
-- -- nécessaires à la maquette "Partage — Vue en lecture seule"
-- (docs/cahier-des-charges/09-maquettes-captures.md, dépôt
-- nexus-jdr-app-mobile) : la ligne "VITESSE / CLASSE D'ARMURE /
-- INSPIRATION" de l'onglet Personnage, ajoutée à la fiche authentifiée par
-- le même chantier ("Inspiration, Vitesse, Classe d'armure affichées sur
-- l'onglet Personnage").
--
-- La Classe d'Armure elle-même n'a besoin d'aucun champ supplémentaire ici
-- : elle est calculée côté client (`ArmorClassCalculator`, dépôt mobile) à
-- partir de `inventory`/`ability_scores`, déjà exposés par cette fonction.
--
-- `is_archived` reste volontairement absent (la vue en lecture seule
-- n'affiche jamais ce statut, propre à la liste de personnages du
-- propriétaire, voir `character_card.dart` côté mobile).
--
-- `race_speed` : `races.speed` n'est lu nulle part ailleurs dans cette
-- fonction (contrairement au nom de race, résolu via `get_translation`) --
-- sous-requête scalaire corrélée dédiée, même style que
-- `weapon_properties`/`armor_properties` dans `inventory` plus bas.
--
-- Copie exacte, caractère pour caractère, du corps de la fonction tel que
-- laissé par 20260908103000 (relu depuis ce fichier avant d'écrire cette
-- migration, pas reconstruit de mémoire) : seules les deux lignes
-- `'inspiration', c.inspiration,` et `'race_speed', (...),` sont nouvelles,
-- tout le reste (skill_id/tool_id/language_id dans leurs tableaux
-- respectifs, feature_uses ET class_features comme deux clés distinctes, la
-- garde `item_id is null` sur la description d'inventaire, alias `it` pour
-- `items`...) est repris à l'identique pour ne rien régresser.
