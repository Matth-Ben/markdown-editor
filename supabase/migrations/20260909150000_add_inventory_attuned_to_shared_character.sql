...
-- ajoute `is_attuned` à chaque entrée `inventory` du jsonb retourné par
-- public.get_shared_character -- nécessaire pour que la vue en lecture
-- seule affiche le même état d'harmonisation que la fiche authentifiée
-- (chantier "Charge portée, harmonisation suivie, tri par catégorie",
-- docs/cahier-des-charges/11-fonctionnalites-a-ajouter.md, section "Onglet
-- Inventaire"). `requires_attunement` (capacité statique de l'objet) était
-- déjà exposé depuis 20260908090000.
--
-- Copie exacte du corps de la fonction tel que laissé par 20260909130000
-- (relu depuis ce fichier avant d'écrire cette migration) : seule la ligne
-- `'is_attuned', ci.is_attuned,` est nouvelle.
