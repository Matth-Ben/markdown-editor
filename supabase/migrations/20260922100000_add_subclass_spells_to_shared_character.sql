-- Chantier "Personnages" — Partage : expose les sorts toujours préparés accordés par les
-- sous-classes (table subclass_spells, migration 20260922090000) dans le jsonb retourné par
-- public.get_shared_character, pour que la vue partagée/MJ les affiche comme la fiche du
-- propriétaire. Seul le bloc 'subclass_spells' est nouveau : il liste, pour chaque classe du
-- personnage qui a une sous-classe, les sorts dont class_level <= niveau de cette classe.
-- Corps de la fonction relu depuis la définition LIVE (pg_get_functiondef) avant écriture.
